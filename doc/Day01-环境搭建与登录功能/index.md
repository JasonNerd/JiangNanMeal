---
title: "瑞吉外卖 - Day01-环境搭建"
date: 2024-01-02T15:39:33+08:00
draft: false
...
瑞吉外卖项目: 一个基于 SpringBoot, MybalisPlus, Redis 的 Java Web 后端开发项目, 它包含管理员网页端和小程序用户端。

对于管理端，业务需求包括员工管理，这包含员工登录与访问权限设定、员工的增删改查与状态修改、员工的条件分页查询。

## 1. 环境准备
### 1-1. MySQL表
一共11个表, 如下为表结构与初始值插入语句.
...
各表名字如下
```cmd
db_reggie.address_book
db_reggie.category
db_reggie.dish
db_reggie.dish_flavor
db_reggie.employee
db_reggie.order_detail
db_reggie.orders
db_reggie.setmeal
db_reggie.setmeal_dish
db_reggie.shopping_cart
db_reggie.user
```

### 1-2. SpringBoot 项目创建
Java-17/SpringBoot-3, 勾选 JavaWeb/lombok/MySQL 相关依赖.

文件夹结构如下:
```bat
Reggie001
    -src/main
        -java/com.rain.reggie
            -Reggie001Application.java
        -resources
            -static
            -templates
            -application.yml
...
#### 配置 application.yml 数据库连接四要素等
```yml
server:
  port: 8080
spring:
  application:
    name: reggie_take_out_A
  datasource:
    druid:
      username: root
      password: root
...
### 1.3 前端页面
将[前端工程](./static.zip)拷贝到 resuorce 文件夹的 static 目录下, 注意不要放错目录. 若不使用 static 目录, 则必须新建配置类指明资源路径映射.
